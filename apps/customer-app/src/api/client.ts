...
const TOKEN_KEY = "neds_customer_auth_token";
...
export async function api<T = any>(path: string, options: {method?: string; body?: any; query?: Record<string, any>; skipAuth?: boolean} = {}): Promise<T> {
  ...
  if (options.query) {
    ...
    Object.entries(options.query).forEach(([k,v]) => { if (v !== undefined && v !== null && v !== "") qs.append(k, String(v)); });
    const value = qs.toString(); if (value) url += `?${value}`;
  }
