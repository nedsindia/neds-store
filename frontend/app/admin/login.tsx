  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!isAdminRole(u.role)) {
        await useAuth; // no-op guard; handled by logout below through context limitation
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
      <View style={[styles.container, !isDesktop && { flexDirection: "column" }]} testID="admin-login-screen">
        {isDesktop && <View style={styles.brandPane}>
          <View style={styles.brandInner}>
            ...
            <View style={styles.brandFeatures}>
              {["Enterprise-grade RBAC", "Hyperlocal delivery orchestration", "Live business rules engine", "Live rider monitoring on Google Maps"].map((label, i) => (
                <View key={label} style={styles.featureRow}><Feather name={["shield", "truck", "activity", "map-pin"][i] as any} size={16} color={theme.colors.primary} /><Text style={styles.featureLabel}>{label}</Text></View>
              ))}
            </View>
          </View>
        </View>}
        <ScrollView style={styles.formPane} contentContainerStyle={styles.formPaneContent} keyboardShouldPersistTaps="handled">
          <View style={styles.formCard} testID="admin-login-form">
            ...
            <Text style={styles.subtitle}>Sign in with an authorized administrator account</Text>
...
const styles = StyleSheet.create({
  ...
  brandPane:{width:"42%",backgroundColor:theme.colors.surface,padding:48,justifyContent:"center"},
  brandInner:{maxWidth:480,gap:12},
  logoBox:{width:56,height:56,borderRadius:12,backgroundColor:theme.colors.primary,alignItems:"center",justifyContent:"center"},
  logoLetter:{color:"#fff",fontSize:28,fontWeight:"800"},
  brandTitle:{color:"#fff",fontSize:36,fontWeight:"800"},
  brandSubtitle:{color:"#A1A1AA",fontSize:14},
  brandTagline:{color:theme.colors.primary,fontSize:13,fontWeight:"700",letterSpacing:1,marginTop:4,marginBottom:32},
  brandFeatures:{gap:14},featureRow:{flexDirection:"row",alignItems:"center",gap:12},featureLabel:{color:"#D4D4D8",fontSize:14},
  formPane:{flex:1,backgroundColor:theme.colors.bg},formPaneContent:{flexGrow:1,alignItems:"center",justifyContent:"center",padding:32},formCard:{width:"100%",maxWidth:420},
  logoBoxSm:{width:48,height:48,borderRadius:10,backgroundColor:theme.colors.primary,alignItems:"center",justifyContent:"center",marginBottom:12},logoLetterSm:{color:"#fff",fontSize:24,fontWeight:"800"},brandTitleSm:{fontSize:22,fontWeight:"800",color:theme.colors.text},
  title:{fontSize:24,fontWeight:"700",color:theme.colors.text},subtitle:{color:theme.colors.textMuted,fontSize:14,marginTop:4}
});
